...
function guardarTransaccion(transaccion){ 
  ...
  if(htmlValue('inputEditar') == "EDICION"){
    ...
    store.getObject().getHistorial().editarTransaccion(indice, transaccion, Transaccion.COMPRA);
  }else{
    console.log("holaaa")
    store.getObject().getHistorial().agregarCompra(transaccion);
  }
  store.save();
}
...
function eliminarTransaccion(indice){ 
  ...
  store.getObject().getHistorial().eliminarTransaccion(indice, Transaccion.COMPRA);
  store.save();
  console.log(store)
}
...
jQuery(document).ready(function($) {
    ...
    htmlEventListener('selectProductos', 'change', habilitarBotonGuardar);
